...
<!--     <form class="flex flex-col" id="ajout-intern-sheep-offer" @submit.prevent="submitForm">
      ...
      <select class="my-4" v-model="offerData.internshipType._id">
        ...
        <option v-for="internshipType in  internshipTypes" :key="internshipType._id" :value="internshipType._id">
          {{ internshipType.value }}</option>
      </select>
...
<script setup>
  ...
  import {
    useEntreprise
  } from '../composables/offres_stage/stageEntreprise';
  import {
    fetchProvinces
  ...
  import {
    useInternshipOffers
  } from '../composables/offres_stage/offreDeStage';
